...
# ---------------------------------------------------------------------
# Auto-download data if none exists
# ---------------------------------------------------------------------
FOLDER_URL = os.getenv(
    "GDRIVE_FOLDER_URL",
    "https://drive.google.com/drive/folders/186OfK2ekqedpEEoNhJM2crhDvWE2uxPp?usp=sharing",
)
...
def _ensure_data():
    ...
    try:
        import gdown
        print("📦 Downloading recipe CSVs from Drive...")
...
def _tokens(q): return re.findall(r"[a-z0-9_\-]+", _norm(q))

def _rarity_score(query):
    toks = _tokens(query)
...
def exact_match_bonus(query, idxs, weight):
...
# AND filter
def _contains_all_terms(row_text, terms):
    toks = set(re.findall(r"[a-z0-9_\-]+", row_text.lower()))
    return all(t in toks for t in terms)

def _filtered_idxs_for_terms(category, ing1, ing2, require_both):
...
# Main ranker
# ---------------------------------------------------------------------
def rank_hybrid_filtered(category, ing1, ing2, ptr,
                         alpha=0.7, k_prefilter=300,
                         strategy="auto",
                         boost_exact=False, boost_weight=0.25,
                         require_both=False):
    idxs = _filtered_idxs_for_terms(category, ing1, ing2, require_both)
    if idxs is None or len(idxs) == 0: return idxs, []

    terms = [t.strip() for t in [ing1, ing2] if t and t.strip()]
    if not terms: return idxs, [ idxs[ptr % len(idxs)] ]
    ...
    if use_two_stage:
        ...
        if boost_exact:
            final_sims += exact_match_bonus(query, idxs_k, boost_weight)
        ...
        final_sims = alpha_eff * sims_b + (1 - alpha_eff) * sims_t
        if boost_exact:
            final_sims += exact_match_bonus(query, idxs, boost_weight)
        ranked = idxs[np.argsort(-final_sims)]

    if len(ranked) == 0: return idxs, []
    return idxs, [ ranked[ptr % len(ranked)] ]

def candidate_total(cat, strat, k_pref, ing1="", ing2="", require_both=False):

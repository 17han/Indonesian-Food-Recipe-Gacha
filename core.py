...
# ---------------------------
# Heuristic classifier (from your Colab)
# ---------------------------
...
# ---------------------------
# Load/merge data from data/*.csv (and auto-category if missing)
# ---------------------------
def _load_merged():
    data_dir = Path("data")
    ...
    for f in files:
        ...
        # if Category missing or blank, auto-classify
        if "Category" not in df.columns:
            df["Category"] = [
                _classify_row(t, i) for t, i in zip(df["Title"], df["Ingredients"])
            ]
        else:
            # fill blanks
